%% virus_phyto_encounter.m
% This script estimates the percentage of phytoplankton cells that would
% encounter a virus of a container of a given volume after a certain
% amount of time.
...
% output: prc_virus = percentage of phytoplankton cells encountering a
% virus
...
function [prc_virus,hit] = virus_phyto_encouter(V, dilution_factor,conc,hv_ratio,diffusion_coeff,dt)

% we calculate how many particles we are dealing with
Np = int32(dilution_factor./100.*V*conc); 
Nv = int32(Np.*hv_ratio);

max_swimspeed = sqrt(diffusion_coeff); 
...
for pp = 1:Np
    ...
    while hit(pp)<1 & dt_count<=dt % as long as we are not hitting the walls or have been hit by a virus
        ...
        %figure(1),hold on,scatter3(tmp_pos(1),tmp_pos(2),tmp_pos(3),'k.')
...
prc_virus = length(find(hit==1))./length(hit)*100;
